    ...
    >
      <div className=" ">
        <div className=" flex items-center justify-start gap-3 pt-5">
          ...
          <p className=" font-inter-semibold text-2xl">{time} мин.</p>
        </div>
        <p className=" font-inter-semibold text-2xl mt-8">{title}</p>
        <ul className=" mt-4 font-inter-regular">
          {services.map((service, index) => (
            <li key={index} className="flex items-center gap-3">
              <MdOutlineCheck />
              <p className=" pb-1">{service}</p>
            </li>
          ))}
        </ul>
      </div>
      <div className=" flex flex-col justify-center items-center">
        <div className="flex flex-row items-center min-w-[180px] min-h-[80px] bg-white-500 w-full mb-6 rounded-3xl py-3">
